...
class Multipart implements Serializer
{
        ...
        foreach ($request->body as $k => $v) {
            ...
            if (is_array($v))
            {
                ...
                if ($v[0] === "json")
                {
                    ...
                    $value_params[] = $this->prepareJsonMultiPart($k, $v[1]);


                }

                else if ($v[0] === "file") {
                    $file_params[] = $this->prepareFileMultiPart($k, $v[1], $v[2]);
                }
    ...
    private function prepareJsonMultiPart($partName, $jsonPart)
    {
